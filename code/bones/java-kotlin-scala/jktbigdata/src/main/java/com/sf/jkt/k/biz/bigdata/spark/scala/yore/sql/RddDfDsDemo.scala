...
object RddDfDsDemo {
  ...
  def testDsDfFilter(): Unit ={
    ...
    //use dynamic structType
     val structType=StructType("uid,sex".split(",").map(field=>StructField(field,StringType,nullable = true)))
     val userRow=userRdd.map(attr=>Row(attr(0),attr(2)))
     val userDf=spark.createDataFrame(userRow,structType)
    userDf.show(5)
    println("-userDf"*10)
    ...
    maleAvgRating.toDF("mid","avgRating").join(movieDf,"id").select("avgRating","name")
        .show(5)
    println("-avgMaleRating"*10)

     maleRating.take(10).foreach(println)
    println("-maleRating"*10)
    spark.stop()
